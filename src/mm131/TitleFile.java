...
public class TitleFile 
{
	public static List<String> getTitles(String url)
	{
		...
		try
		{
			...
			Element nav = doc.select(".list-left.public-box").last();			
			Elements lis = nav.getElementsByTag("dd");
			...
			for (;i < lis.size() -1 ;i++) 
			{
				  ...
				  for(int j = 0 ; j < aset.size() ; j++)
				  {
					  ...
					  String title = aa.child(0).attr("alt");
	...
	public static List<String> getPages(String url,int type,int num)
	{
		List<String> lsttitles = new LinkedList<String>();
		try
		{			
			...
			for (i = 1 ; i <= num ; i++) 
			{
				...
				{
					lsttitles.add(url + "list_" + type + "_" + i + ".html");
				}
